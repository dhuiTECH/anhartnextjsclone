...
// ICON IMPORTS
// =============================================================================
import { Home as HomeIcon, Heart, Globe, ExternalLink, ArrowRight, ArrowLeft, X } from "lucide-react";
...
// =============================================================================
import { organizationStructuredData, websiteStructuredData, faqStructuredData, localBusinessStructuredData, provinceBusinessSchemas } from "@/lib/structuredData";
...
const Home = () => {
  ...
  const {
    count: capitalCount,
    elementRef: capitalRef
  } = useCountUp({
    end: 20 // Patient capital in millions
  });
  const {
    count: partnersCount,
    elementRef: partnersRef
  } = useCountUp({
  ...
  const handleViewDetails = (projectData: ProjectData) => {
    ...
    const portfolioProject = portfolioProjects.find(project => project.title === projectData.originalTitle || project.title === projectData.title || projectData.originalTitle && project.title.includes(projectData.originalTitle) || projectData.title && project.title.includes(projectData.title));
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main>
            ...
            <ScrollAnimationWrapper direction="top" delay={200}>
              <p className="text-base sm:text-lg lg:text-xl leading-7 text-gray-700/95 mb-8">
                ...
                <span className="block sm:hidden">
                  From our first project in Vancouver to communities nationwide, Anhart partners with local non-profits
                  to deliver 20,000 affordable homes by 2045 — proving that small, trusted collaborations can achieve
                  big goals.
                </span>

                {/* Tablet/Desktop text */}
                <span className="hidden sm:block text-xl leading-8 text-muted-foreground mb-8">
                  From our first project in Vancouver's Downtown Eastside in 2000, Anhart has grown into one of Canada's
                  leading non-profit housing developers. We are on track to deliver 20,000 affordable homes nationwide
                  by 2045, working coast to coast to coast. Our model combines patient capital, trusted local
                  partnerships, and the latest insights from complex adaptive systems. To us, <strong>“small”</strong>{" "}
                  means staying at scale while remaining deeply rooted in local networks — a distributed approach where
                  each community functions like a vital cell in a living system. Together, these small, connected
                  efforts form the larger organism of affordable housing, proving that the power of small is the most
                  effective pathway to achieving big goals.
                </span>
              </p>
            </ScrollAnimationWrapper>
            {/* Statistics */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
              <div className="flex flex-col items-center cursor-pointer hover:scale-105 transition-all duration-300 animate-scale-in delay-300 group" onClick={handleOpenGallery} role="button" tabIndex={0} onKeyDown={e => {
              if (e.key === "Enter" || e.key === " ") {
                ...
                <div className="bg-primary/10 p-4 rounded-full mb-4 group-hover:bg-primary/20 group-hover:scale-110 transition-all duration-300 shadow-2xl shadow-primary/30">
                  <HomeIcon className={`w-8 h-8 text-primary group-hover:text-primary/80 transition-all duration-300 drop-shadow-[0_8px_16px_rgba(0,0,0,0.6)] drop-shadow-[0_4px_8px_rgba(0,0,0,0.4)] drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)] drop-shadow-[0_0_0_4px_rgba(255,255,255,1)] ${showStatsAnimation ? "animate-[bounce_1s_ease-in-out]" : ""}`} />
                </div>
                <p ref={homesRef} className="text-3xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors duration-300">
                  {homesCount}+
                </p>
              ...
              <div className="flex flex-col items-center cursor-pointer hover:scale-105 transition-all duration-300 animate-scale-in delay-400 group" onClick={handleOpenMetrics} role="button" tabIndex={0} onKeyDown={e => {
              if (e.key === "Enter" || e.key === " ") {
                e.preventDefault();
                handleOpenMetrics();
              }
                ...
                <div className="bg-primary/10 p-4 rounded-full mb-4 group-hover:bg-primary/20 group-hover:scale-110 transition-all duration-300 shadow-2xl shadow-primary/30">
                  <Heart className={`w-8 h-8 text-primary group-hover:text-primary/80 transition-all duration-300 drop-shadow-[0_8px_16px_rgba(0,0,0,0.6)] drop-shadow-[0_4px_8px_rgba(0,0,0,0.4)] drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)] drop-shadow-[0_0_0_4px_rgba(255,255,255,1)] ${showStatsAnimation ? "animate-[bounce_1s_ease-in-out]" : ""}`} />
                </div>
                <p ref={capitalRef} className="text-3xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors duration-300">
                  ${capitalCount}M
                </p>
                <p className="text-muted-foreground group-hover:text-foreground transition-colors duration-300">
                  Patient Capital
              ...
              <div className="flex flex-col items-center cursor-pointer hover:scale-105 transition-all duration-300 animate-scale-in delay-500 group" onClick={openClientsDialog} role="button" tabIndex={0} onKeyDown={e => {
              if (e.key === "Enter" || e.key === " ") {
                e.preventDefault();
                openClientsDialog();
              }
                ...
                <div className="bg-primary/10 p-4 rounded-full mb-4 group-hover:bg-primary/20 group-hover:scale-110 transition-all duration-300 shadow-2xl shadow-primary/30">
                  <Globe className={`w-8 h-8 text-primary group-hover:text-primary/80 transition-all duration-300 drop-shadow-[0_8px_16px_rgba(0,0,0,0.6)] drop-shadow-[0_4px_8px_rgba(0,0,0,0.4)] drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)] drop-shadow-[0_0_0_4px_rgba(255,255,255,1)] ${showStatsAnimation ? "animate-[bounce_1s_ease-in-out]" : ""}`} />
                </div>
                <p ref={partnersRef} className="text-3xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors duration-300">
                  {partnersCount}+
                </p>
                <p className="text-muted-foreground group-hover:text-foreground transition-colors duration-300">
                  Global Partners
        ...
        <Dialog open={clientsDialogOpen} onOpenChange={setClientsDialogOpen}>
          <DialogContent className="w-[95vw] max-w-4xl p-0 overflow-hidden dialog-content">
            <div className="relative">
              <Button variant="ghost" size="icon" className="absolute right-2 top-2 z-10 bg-background/80 hover:bg-background" onClick={closeClientsDialog}>
                <X className="h-4 w-4" />
              </Button>
              <ClientCarousel />
            </div>
        ...
        <ThreeCardSection title="How do we do it?" description="Our Approach: Expertise, Altruism, Research, and Community Housing Leadership" cards={howDoWeDoItPillars} bgColor="muted" layout="grid" showImages={true} flippable={true} />
        ...
        <section className="py-16 bg-muted/30">
          <div className="mx-auto max-w-6xl px-6 lg:px-8">
            <div className="text-center mb-12">
              <ScrollAnimationWrapper direction="top">
                <h2 className="text-3xl font-bold tracking-tight text-foreground mb-4">Contact Us</h2>
              </ScrollAnimationWrapper>
              <ScrollAnimationWrapper direction="top" delay={100}>
                <p className="text-lg text-muted-foreground">Reach out to connect with our friendly-staff or call us at <a href="tel:604-529-6259" className="font-bold text-foreground hover:text-primary transition-colors">604-529-6259</a></p>
              </ScrollAnimationWrapper>
            </div>
                      ...
                      <div className="flex-1 space-y-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input id="name" name="name" placeholder="Your full name" className="mt-1" required />
                          </div>
                          <div>
                            <Label htmlFor="email">
                              Email <span className="text-destructive">*</span>
                            </Label>
                            <Input id="email" name="email" type="email" placeholder="your.email@example.com" className="mt-1" required />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <Input id="organization" name="organization" placeholder="Your organization name" className="mt-1" />
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <Input id="subject" name="subject" placeholder="Subject of your message" className="mt-1" required />
                        </div>
                        ...
                        <div className="flex-1">
                          ...
                          <Textarea id="message" name="message" placeholder="Tell us about your project or how we can help..." required className="mt-1 min-h-[200px] resize-none" />
                        </div>
              ...
              <div className="flex flex-col gap-6 h-full">
                <ScrollAnimationWrapper direction="right" delay={300}>
                  <Card className="flex-1 hover:shadow-md transition-shadow duration-300">
                    <CardContent className="p-8 h-full">
                      <h3 className="text-xl font-semibold text-foreground mb-6">Main Office</h3>
                      <div className="space-y-4 text-muted-foreground">
                        <p className="flex items-start">
                          <HomeIcon className="w-5 h-5 text-primary mr-3 mt-0.5 flex-shrink-0" />
                          <button onClick={() => openGoogleMapsSearch(AddressUtils.getGoogleMapsAddress())} className="hover:underline text-left focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 rounded">
                        ...
                        <p className="flex items-center">
                          ...
                          <a href="mailto:info@anhart.ca" className="hover:underline">
                  ...
                  <Card className="flex-1 hover:shadow-md transition-shadow duration-300 hidden md:block">
                    <CardContent className="p-8 h-full flex flex-col">
                      ...
                      <p className="text-muted-foreground mb-6 flex-grow">
                        We'd love to hear about your housing project or partnership opportunity. Our team is ready to
                        work with you to create sustainable, affordable housing solutions.
                      </p>
      ...
      <ProjectGalleryModal isOpen={galleryModalOpen} onClose={handleCloseGallery} projects={portfolioProjects} />
